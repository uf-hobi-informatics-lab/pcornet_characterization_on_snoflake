CREATE OR REPLACE PROCEDURE CHARACTERIZATION.DCQ_CHECKS.SP_DC_3_05"("DB_PARAM" VARCHAR, "SCHEMA_NAME" VARCHAR, "RUN_ID" VARCHAR, "TARGET_TABLE" VARCHAR, "PREV_DB_PARAM" VARCHAR, "PREV_SCHEMA_NAME" VARCHAR, "START_DATE" VARCHAR, "END_DATE" VARCHAR)
RETURNS VARCHAR
LANGUAGE JAVASCRIPT
EXECUTE AS CALLER
AS '
...
if (!(only === "ALL" || only === "ENCOUNTER" || only === "PROCEDURES")) {
  throw new Error(`Invalid TARGET_TABLE=''${TARGET_TABLE}''. Use ALL, ENCOUNTER, or PROCEDURES.`);
}
...
if (!tableExists(DB_PARAM, SCHEMA_NAME, "ENCOUNTER") || !colExists(DB_PARAM, SCHEMA_NAME, "ENCOUNTER", "PATID")) {
  ...
  return `DC 3.05 ERROR: ENCOUNTER missing`;
}
if (!tableExists(DB_PARAM, SCHEMA_NAME, "PROCEDURES") || !colExists(DB_PARAM, SCHEMA_NAME, "PROCEDURES", "PATID")) {
...
const rs = q(
  ...
  SELECT
    ...
    (SELECT COUNT(*) FROM p) AS procedures_patid_distinct_n
  `
);
rs.next();
...
const details = {
  ...
  procedures_patid_distinct_n: numer,
  procedures_coverage_pct: pct,
...
insertMetric(resultsTbl, base, "PROCEDURES_PATID_DISTINCT_N", numer, String(numer), thresholdPct, false, details);
insertMetric(resultsTbl, base, "PROCEDURES_COVERAGE_PCT", pct, String(pct), thresholdPct, false, details);
insertMetric(resultsTbl, base, "PROCEDURES_COVERAGE_FLAG", flag, String(flag), thresholdPct, (flag === 1), details);
return `DC 3.05 finished RUN_ID=${RUN_ID} TARGET_TABLE=${only}`;
';