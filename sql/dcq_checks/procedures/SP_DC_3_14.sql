CREATE OR REPLACE PROCEDURE CHARACTERIZATION.DCQ_CHECKS.SP_DC_3_14"("DB_PARAM" VARCHAR, "SCHEMA_NAME" VARCHAR, "RUN_ID" VARCHAR, "TARGET_TABLE" VARCHAR, "PREV_DB_PARAM" VARCHAR, "PREV_SCHEMA_NAME" VARCHAR, "START_DATE" VARCHAR, "END_DATE" VARCHAR)
RETURNS VARCHAR
LANGUAGE JAVASCRIPT
EXECUTE AS CALLER
AS '
...
if (!(only === ''ALL'' || only === ''MED_ADMIN'' || only === ''DISPENSING'' || only === ''OBS_CLIN'')) {
  throw new Error(`Invalid TARGET_TABLE=''${TARGET_TABLE}''. Use ALL, MED_ADMIN, DISPENSING, or OBS_CLIN.`);
}

// SAS Table IVJ: check Month -3 only, flag if count=0 or <75% of benchmark average (months -12..-23 avg).
...
const defs = [
  { table: ''MED_ADMIN'', dateCol: ''MEDADMIN_START_DATE'' },
  { table: ''DISPENSING'', dateCol: ''DISPENSE_DATE'' },
  { table: ''OBS_CLIN'', dateCol: ''OBSCLIN_START_DATE'' }
];
...
return `DC 3.14 finished RUN_ID=${RUN_ID} TARGET_TABLE=${only}`;
';