CREATE OR REPLACE PROCEDURE CHARACTERIZATION.DCQ_CHECKS.SP_DC_2_02"("DB_PARAM" VARCHAR, "SCHEMA_NAME" VARCHAR, "RUN_ID" VARCHAR, "TARGET_TABLE" VARCHAR, "PREV_DB_PARAM" VARCHAR, "PREV_SCHEMA_NAME" VARCHAR, "START_DATE" VARCHAR, "END_DATE" VARCHAR)
RETURNS VARCHAR
LANGUAGE JAVASCRIPT
EXECUTE AS CALLER
AS '
...
for (const c of selected) {
  ...
  const sql = `
    WITH v AS (
      ...
      FROM ${fullTable}
      WHERE TRY_TO_DOUBLE(${c.field}) IS NOT NULL${dateFilterWhere(c.table)}
    )
    SELECT
      COUNT(*) AS denom,
